...
async function exercise2(landmark) {
  ...
  //if ((calculate_distance(lPinky, rElbow) < 0.05) && (calculate_distance(rPinky, lElbow) < 0.05) && (lPinky.z < rElbow.z) && (rPinky.z < lElbow.z)) {
    ...
    if (stage === 'Upper') {
      ...
      if (count2 % 10 < 5) {
          //lShoulder = landmark[11]
          if ((Math.abs(lShoulder.y - rElbow.y) <= 0.15 * torsoDiagonal) && (rElbow.x > lShoulder.x)) {
          ...
          //rShoulder = landmark[12]
          if ((Math.abs(rShoulder.y - lElbow.y) <= 0.15 * torsoDiagonal) && (lElbow.x < rShoulder.x)) {
              message = 'Опустите скрещенные руки в исходное положение'
              stage = 'Lower'
              toScore = true
            }
      ...
      if (calculate_distance(centerElbow, centerBottom) <= 0.15 * torsoDiagonal) {
        ...
        if (count2 % 10 < 5) {
          message = 'Поднимите скрещенные руки до уровня плеч c поворотом влево';
        }
        else {
          message = 'Поднимите скрещенные руки до уровня плеч c поворотом вправо';
        }
      }
    }
  }
...
async function exercise3(landmark) {
  ...
  if (count3 % 10 < 5) {
      ...
      if (calculate_distance(rIndex, lWrist) >= 0.15 * torsoDiagonal)  {
          message = 'Обхватите кистью правой руки запястье левой и опустите их вниз'
          stage = 'Lower'
          toScore = false
      } else {
          if (stage === 'Upper') {
              if ((Math.abs(lShoulder.y - rElbow.y) <= 0.15 * torsoDiagonal) && (rIndex.x > lShoulder.x)) {
                  if (rElbowAngle >= 160) {
      ...
      if (calculate_distance(lIndex, rWrist) >= 0.15 * torsoDiagonal)  {
          message = 'Обхватите кистью левой руки запястье правой и опустите их вниз'
          stage = 'Lower'
          toScore = false
      } else {
          if (stage === 'Upper') {
              if ((Math.abs(rShoulder.y - lElbow.y) <= 0.15 * torsoDiagonal) && (lIndex.x < rShoulder.x)) {
                  if (lElbowAngle >= 160) {
                      message = 'Опустите руки в исходное положение'
                      stage = 'Lower'
                      toScore = true
